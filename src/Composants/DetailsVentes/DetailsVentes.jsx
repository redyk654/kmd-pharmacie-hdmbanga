...
export default function DetailsVentes(props) {
    ...
    useEffect(() => {
        ...
        if (dateDepart.length > 0 && dateFin.length > 0) {
            ...
            if (props.role !== admin) {
                req.open('POST', `http://serveur/backend-cma/details_ventes.php?vendeur=${props.nomConnecte}`);
            } else {
                if (filtre) {
                    req.open('POST', `http://serveur/backend-cma/details_ventes.php?vendeur=${caissier}`);
                } else {
                    req.open('POST', `http://serveur/backend-cma/details_ventes.php`);
                }
            }
                        ...
                        <tbody>
                            {historique.length > 0 ? historique.map(item => (
                                <tr>
                                    ...
                                    <td>{item.quantite}</td>
                                    <td>{parseInt(item.prix_total)/parseInt(item.quantite) + ' Fcfa'}</td>
